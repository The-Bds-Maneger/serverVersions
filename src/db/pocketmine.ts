...
const bucket = await coreUtils.oracleBucket("sa-saopaulo-1", "bdsFiles", "grwodtg32n4d", "0IKM-5KFpAF8PuWoVe86QFsF4sipU2rXfojpaOMEdf4QgFQLcLlDWgMSPHWmjf5W");
...
app.get("/bin", async (req, res) => {
  ...
  const files = (await bucket.fileList()).filter(({name}) => name.startsWith("/php_bin/"));
