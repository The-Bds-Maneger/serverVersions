...
async function Find() {
  const { document } = (new jsdom.JSDOM(await httpRequest.bufferRequest("https://getbukkit.org/download/spigot").then(res => res.body.toString("utf8")).catch(err => {console.log(err); return "<html></html>"}))).window;
  ...
  const Versions = await Promise.all(([...dooms]).map(async DOM => {
    const download = (new jsdom.JSDOM(await httpRequest.bufferRequest(DOM.querySelector("div > div.col-sm-4 > div.btn-group > a")["href"]).then(res => res.body.toString("utf8"))));
    const serverInfo = {
