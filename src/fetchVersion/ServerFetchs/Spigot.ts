...
async function Add(Version: string, versionDate: Date, url: string) {
  if (await spigot.findOne({ version: Version }).lean().then(data => !!data).catch(() => true)) console.log("Spigot: version (%s) already exists", Version);
  else {
    console.log("Spigot: Version %s, url %s", Version, url);
    await spigot.create({
      version: Version,
